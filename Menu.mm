...
static NSString *ImagePath(NSString *fileName) {
   NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
   if(paths.count == 0)
      return @"んまmod";

   NSString *directory = [[paths objectAtIndex:0] stringByAppendingPathComponent:@"んまmod"];
   return [directory stringByAppendingPathComponent:fileName];
}

typedef bool (*KukioCTFontRegisterGraphicsFontFn)(CGFontRef font, CFErrorRef *error);

static KukioCTFontRegisterGraphicsFontFn KukioCTFontRegisterGraphicsFont() {
   static BOOL attempted = NO;
   static KukioCTFontRegisterGraphicsFontFn function = NULL;
   if(attempted)
      return function;
   ...
   function = (KukioCTFontRegisterGraphicsFontFn)dlsym(handle, "CTFontManagerRegisterGraphicsFont");
   return function;
}

static NSString *KukioMenuFontName() {
   static BOOL attempted = NO;
   ...
   for(uint32_t i = 0; i < imageCount; i++) {
      ...
      KukioCTFontRegisterGraphicsFontFn registerFont = KukioCTFontRegisterGraphicsFont();
...
static UIFont *KukioMenuFont(CGFloat size) {
   NSString *fontName = KukioMenuFontName();
   UIFont *font = fontName.length > 0 ? [UIFont fontWithName:fontName size:size] : nil;
   return font ?: ([UIFont fontWithName:@"AppleSDGothicNeo-Light" size:size] ?: [UIFont systemFontOfSize:size]);
}

static NSString *KukioSliderValueText(float value, BOOL isFloating, float maxValue) {
   if(!isFloating)
...
static NSString * const kMenuButtonPositionXKey = @"KukioMod_MenuButtonPositionX";
static NSString * const kMenuButtonPositionYKey = @"KukioMod_MenuButtonPositionY";

static CGPoint DefaultMenuButtonPosition(UIWindow *window, BOOL appIsPortrait, CGSize buttonSize) {
   float menuButtonX = window.frame.size.width - buttonSize.width - 30;
   float menuButtonY = 30;
   if(!appIsPortrait){
      menuButtonX = window.frame.size.width - buttonSize.width - 5;
      menuButtonY = 5;
   }

   return CGPointMake(menuButtonX, menuButtonY);
}

static BOOL MenuButtonFrameIsOnScreen(UIWindow *window, CGRect frame) {
   if(window == nil || CGRectIsEmpty(frame))
      return NO;

   return CGRectContainsRect(window.bounds, frame);
}

static CGPoint RestoredMenuButtonPositionOrDefault(NSUserDefaults *defaults, UIWindow *window, BOOL appIsPortrait, CGSize buttonSize) {
   CGPoint defaultPosition = DefaultMenuButtonPosition(window, appIsPortrait, buttonSize);
   id savedX = [defaults objectForKey:kMenuButtonPositionXKey];
   id savedY = [defaults objectForKey:kMenuButtonPositionYKey];
   if(savedX == nil || savedY == nil)
      return defaultPosition;

   CGPoint savedPosition = CGPointMake([(NSNumber *)savedX doubleValue], [(NSNumber *)savedY doubleValue]);
   CGRect savedFrame = CGRectMake(savedPosition.x, savedPosition.y, buttonSize.width, buttonSize.height);
   if(MenuButtonFrameIsOnScreen(window, savedFrame))
      return savedPosition;

   [defaults removeObjectForKey:kMenuButtonPositionXKey];
   [defaults removeObjectForKey:kMenuButtonPositionYKey];
   [defaults synchronize];
   return defaultPosition;
}

static void SaveMenuButtonPosition(NSUserDefaults *defaults, CGPoint position) {
   [defaults setObject:@(position.x) forKey:kMenuButtonPositionXKey];
   [defaults setObject:@(position.y) forKey:kMenuButtonPositionYKey];
   [defaults synchronize];
}
...
- (void)addPageItem:(NSString *)title_ targetPage:(NSUInteger)targetPage_
{
   ...
   //Arrow image
   NSString *desiredImgPath = ImagePath(@"in.png");
...
-(void)menuSliderValueChanged:(MyUISlider *)slider_ 
{
   ...
   dispatch_async(dispatch_get_main_queue(), ^{
      ...
      sliderLabel.text = KukioSliderValueText(sliderViewRef.value, sliderItem.IsFloating, sliderItem.MaxValue);
		...
		description:(NSString *)description_ isOn:(BOOL)isOn_ isFloating:(BOOL)isFloating_ defaultValue:(float)defaultValue_ minValue:(float)minValue_ maxValue:(float)maxValue_
{
   ...
   NSString *desiredImgPath = ImagePath(@"sliderIcon.png");
   ...
   dispatch_async(dispatch_get_main_queue(), ^{
      menuSliderValue.text = KukioSliderValueText(menuSlider.value, isFloating_, maxValue_);
   });

   menuSliderValue.textColor = [UIColor whiteColor];
   menuSliderValue.font = KukioMenuFont(15);
...
{   
   ...
   myTextfield.font = KukioMenuFont(14);
...
- (void)moveMenuButton:(UIPanGestureRecognizer *)gesture {
   ...
   if(gesture.state == UIGestureRecognizerStateEnded)
   {
      ...
      SaveMenuButtonPosition(userDefaults, latestMenuButtonPosition);
   }
}
...
- (id)initMenu{
   ...
   //Menu button
   NSString *desiredImgPath = ImagePath(@"menuIcon.png");
   UIImage* menuButtonImage = [UIImage imageWithContentsOfFile:desiredImgPath];   
   menuButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];

   CGSize menuButtonSize = CGSizeMake(50, 50);
   CGPoint menuButtonPosition = RestoredMenuButtonPositionOrDefault(userDefaults, mainWindow, appIsPortrait, menuButtonSize);
   float menuButtonX = menuButtonPosition.x;
   float menuButtonY = menuButtonPosition.y;
   
   latestMenuButtonPosition.x = menuButtonX;
   latestMenuButtonPosition.y = menuButtonY;
   menuButton.frame = CGRectMake(menuButtonX, menuButtonY, menuButtonSize.width, menuButtonSize.height);
   ...
   menuTitle.text = @"んまmod";
   menuTitle.textColor = [UIColor whiteColor];
   menuTitle.font = KukioMenuFont(17.0f);
   ...
   NSString *desiredImgPathBack = ImagePath(@"back.png");
   ...
   //Menu close button
   NSString *desiredImgPathClose = ImagePath(@"close.png");
